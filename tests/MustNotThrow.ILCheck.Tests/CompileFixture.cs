...
// copying CallgraphClosure.Attributes.dll alongside so Cecil can resolve the attribute.
public static class CompileFixture
{
    public static string Emit(string source, string assemblyName = "Fixture")
    {
        var tempDir = Path.Combine(Path.GetTempPath(), "cgc-il-fixtures-mnt", Guid.NewGuid().ToString("N"));
        ...
        var attributesDllPath = typeof(global::CallgraphClosure.Attributes.MustNotThrowAttribute)
            .Assembly.Location;
        File.Copy(
            attributesDllPath,
    ...
    private static IEnumerable<MetadataReference> GetStandardReferences()
    {
        ...
        yield return MetadataReference.CreateFromFile(
            typeof(global::CallgraphClosure.Attributes.MustNotThrowAttribute).Assembly.Location);
    }
}
